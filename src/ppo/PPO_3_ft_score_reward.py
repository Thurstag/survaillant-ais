...
# But the reward is now based on
# the game score (chests, monsters, combo, ...)

SERVER_URL = "http://localhost:4000/"


def reset_game():
    requests.get(SERVER_URL + "createNewGame/0")
...
def move_player(actions):
    ...
    if rep == "BAD_MOVEMENT":
        ...
        new_score = get_scores()["totalScore"]
...
value = tf.squeeze(mlp(observation_input, list(
    hidden_sizes) + [1], tf.tanh, None), axis=1)
...
try:
    actor.load_weights('./modelsweight_ppo3/cartpole_actor.h5')
    critic.load_weights('./modelsweight_ppo3/cartpole_critic.h5')
except OSError as e:
    pass
...
for epoch in range(epochs):
    ...
    # Save the model
    actor.save("modelsweight_ppo3/cartpole_actor.h5")
    critic.save("modelsweight_ppo3/cartpole_critic.h5")
