...
const STEPS_PER_EPOCH = 100, DDPG_TURNS = 111;
...
async function assertTrainingExport(networks, map, isFlashlight, epochs, rewardPolicy, state, representation, itemsEnabled, agent) {
...
describe("Training integration tests", () => {
                ...
                for (const items of [ true, false ]) {
                    it(`Train PPO network (policy: ${rewardPolicy}, representation: ${representation}, state: ${state}, items: ${items})`, async function () {
                        ...
                        await assertTrainingExport([ PPO_POLICY_NETWORK_NAME, PPO_VALUE_NETWORK_NAME ], map, isFlashlight, args[PpoArgument.EPOCHS],
                            args[PpoArgument.POLICY], args[PpoArgument.STATE_MODE], args[PpoArgument.REPRESENTATION], args[PpoArgument.ITEMS], PpoAgent.ID);
                    });
                    ...
                    it(`Train DDPG network (policy: ${rewardPolicy}, representation: ${representation}, state: ${state}, items: ${items})`, async function () {
                        ...
                        args[DdpgArgument.ITEMS] = items;
                        args[DdpgArgument.EPOCHS] = DDPG_EPOCHS;
                        args[DdpgArgument.TURNS_LIMIT] = DDPG_TURNS;
                        ...
                        args[DdpgHyperparameter.BUFFER_CAPACITY] = Math.floor(args[DdpgArgument.EPOCHS] / 2);
                        ...
                        await assertTrainingExport([ DDPG_ACTOR_NETWORK_NAME, DDPG_CRITIC_NETWORK_NAME ], map, isFlashlight, args[DdpgArgument.EPOCHS],
                            args[DdpgArgument.POLICY], args[DdpgArgument.STATE_MODE], args[DdpgArgument.REPRESENTATION], args[DdpgArgument.ITEMS], DdpgAgent.ID);
                    });
                }
            }
        }
    ...
    for (const items of [ true, false ]) {
        it(`Train an existing PPO network (items: ${items})`, async function () {
            ...
            args[PpoArgument.STATS_FOLDER] = TMP_DIRECTORY;
            args[PpoHyperparameter.STEPS_PER_EPOCH] = STEPS_PER_EPOCH;
            args[PpoArgument.BASE_NETWORK_FOLDER] = path.join(__dirname, "assets", items ? "ppo_items" : "ppo");
            ...
            for (const network of [ PPO_POLICY_NETWORK_NAME, PPO_VALUE_NETWORK_NAME ]) {
                await assertNetworkFiles(path.join(TMP_DIRECTORY, `${network}${SurvaillantTrainingNetwork.SAVED_MODEL_EXTENSION}`),
                    PpoAgent.ID, args[PpoArgument.EPOCHS], args[PpoArgument.POLICY], args[PpoArgument.STATE_MODE], stateParams, args[PpoArgument.REPRESENTATION], args[PpoArgument.ITEMS], [ map ]);
            }
        });
        ...
        it(`Train an existing DDPG network (items: ${items})`, async function () {
            ...
            args[DdpgArgument.NETWORK_FOLDER] = TMP_DIRECTORY;
            args[DdpgArgument.STATS_FOLDER] = TMP_DIRECTORY;
            args[DdpgArgument.BASE_NETWORK_FOLDER] = path.join(__dirname, "assets", items ? "ddpg_items" : "ddpg");
            ...
            for (const network of [ DDPG_ACTOR_NETWORK_NAME, DDPG_CRITIC_NETWORK_NAME ]) {
                await assertNetworkFiles(path.join(TMP_DIRECTORY, `${network}${SurvaillantTrainingNetwork.SAVED_MODEL_EXTENSION}`),
                    DdpgAgent.ID, args[DdpgArgument.EPOCHS], args[DdpgArgument.POLICY], args[DdpgArgument.STATE_MODE], stateParams, args[DdpgArgument.REPRESENTATION], args[DdpgArgument.ITEMS], [ map ]);
            }
        });
        ...
        it(`Train PPO on multiple maps (items: ${items})`, async function () {
            ...
            for (const network of [ PPO_POLICY_NETWORK_NAME, PPO_VALUE_NETWORK_NAME ]) {
                await assertNetworkFiles(path.join(TMP_DIRECTORY, `${network}${SurvaillantTrainingNetwork.SAVED_MODEL_EXTENSION}`),
                    PpoAgent.ID, args[PpoArgument.EPOCHS], args[PpoArgument.POLICY], args[PpoArgument.STATE_MODE], stateParams, args[PpoArgument.REPRESENTATION], args[PpoArgument.ITEMS], MAP_PATHS);
            }
        });
        ...
        it(`Train DDPG on multiple maps (items: ${items})`, async function () {
            ...
            for (const network of [ DDPG_ACTOR_NETWORK_NAME, DDPG_CRITIC_NETWORK_NAME ]) {
                await assertNetworkFiles(path.join(TMP_DIRECTORY, `${network}${SurvaillantTrainingNetwork.SAVED_MODEL_EXTENSION}`),
                    DdpgAgent.ID, args[DdpgArgument.EPOCHS], args[DdpgArgument.POLICY], args[DdpgArgument.STATE_MODE], stateParams, args[DdpgArgument.REPRESENTATION], args[DdpgArgument.ITEMS], MAP_PATHS);
            }
        });
    }
});
